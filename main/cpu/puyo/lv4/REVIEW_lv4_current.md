...
```
cpu4_weights.js                cpu4.cpp                         build/build.cpp
─────────────────              ────────────────                ──────────────────────────
...
- **重みの34要素対応表**は `cpu4.cpp:49-100` と `cpu4_weights.js:144-194` が一対一で対応。`ojamaCount` による発火閾値の**動的緩和**は JS 側（`cpu4_weights.js:148-157`）で `[7]/[9]` に焼き込む（※後述の通り amaEvalMode=1 では実質未使用）。
...
`free / fast / allClear` モードはディレクトリだけ存在し未実装（README のみ）。`cpu4.cpp:107` で常に build へ振り分ける。
...
**現行は amaEvalMode=1 が有効**（`cpu4_weights.js:109`）なので、実際に走るのは次の早期 return パスのみ:
...
3. **quiescence のコスト**: `stepDepth` の各配置ごとに `calcQuiescenceEval`（列×色×最大3ドロップ＋`simulateChain`）が走る。ビーム幅×深さ×全配置に対して効くため、速度のホットスポット。memory の `project_cpu4_perf_backlog` #3（postPot/prePot のbeam深さ間キャッシュ）が未着手。
