...
//
//   ★ 各重みの詳細な意味・由来・チューニング指針は cpu4_weights.md を参照。
//     JS には各値の末尾に1行の要約コメントだけを残す。
// ─────────────────────────────────────────────

Object.assign(window.PuyoCPU4.prototype, {

    // ★ 評価パラメータの初期化（3グループの詳細は cpu4_weights.md「重みの3グループ」）。
    ...
    _initWeights() {
        ...
        this.evalWeights = {
            ...
            // ── Ama 由来の評価値（詳細: cpu4_weights.md）。加算式・0で無効化可 ──
            ...
            // ── quiescence remain の連結数（次連鎖の種。詳細: cpu4_weights.md）──
            qLink2Weight:           4,  // quiescence remain の2連結
            qLink3Weight:          12,  // quiescence remain の3連結（次連鎖の種）

            sideWeight:             0,  // 致死列(第3列) side bias（0で無効。詳細: cpu4_weights.md）

            tearWeight:           -20,  // ちぎりペナルティ（負。0で無効。詳細: cpu4_weights.md）

            wasteWeight:           -2,  // ama型 waste ペナルティ（負。amaEvalMode=1 のみ作用。詳細: cpu4_weights.md）

            formWeight:            50,  // 関係性 form 一致(GTR/SGTR/FRON)（0で無効。詳細: cpu4_weights.md）

            // ── 3連結の形状別倍率（百分率。link3Weight/qLink3Weight に共通で乗る。詳細: cpu4_weights.md）──
        ...
        this.controlWeights = {
            ...
            ignitionThreshold:     10,  // 発火閾値（旧eval=amaEvalMode=0専用。ama型では未参照。詳細: cpu4_weights.md）
            ignitionScoreThreshold: 1000, // 発火スコア閾値（旧eval専用。ama型では未参照。詳細: cpu4_weights.md）
            emergencyHeight:       12,  // 緊急回避ライン

            // ── 初手選択：連鎖スコア主体の「同点崩しband」（詳細: cpu4_weights.md）──
            expChainWeight:      1500,  // 同点崩しband（連鎖スコア単位。0=厳密連鎖最大）

            // ── 確定NEXTビームの探索コスト（速度調整。詳細: cpu4_weights.md）──
            expBranch:             1,  // ※未使用（擬似分岐撤去済み・配線のみ残置）
            expMaxDepth:           8,  // 確定NEXTビームの深さ 1..8（0=8）
            expBeamWidth:          10,  // depth>=1 のビーム幅（0=従来テーパ）

            pruneChainScore:     3000,  // 発火枝刈り(PRUNE)閾値（0=無効。詳細: cpu4_weights.md）

            // ── eval スコア関数の A/B 切替（核心。詳細: cpu4_weights.md）──
            amaEvalMode:           1,  // 1=ama型eval / 0=旧eval(calcRewardScore)。探索構造は不変

            // ── 発火トリガ（fire gate。「いつ撃つか」を初手選択直前で決める。詳細: cpu4_weights.md）──
            ...
            growthFireForbidChains: 2,  // 育成こぼし抑制：発火を許す段数の下限を除外（0=無効/1=全こぼし禁止。詳細: cpu4_weights.md）

            // ── 緊急発火の発火対象制限（本線巻き込み防止。詳細: cpu4_weights.md）──
        ...
        // ── モード別プロファイルの土台（詳細: cpu4_weights.md「モードプロファイル」）──
    ...
    // ★ モード別プロファイル（build からの差分上書き定義。詳細: cpu4_weights.md「モードプロファイル」）。
    ...
    _modeProfiles() {
        return {
            ...
            // fast：速攻型（「多少汚くても浅い連鎖を速く返す」。詳細: cpu4_weights.md）。
            fast: {
                evalWeights: {
    ...
    //   インデックス順は下の各行末 [N] と一致させること（全体表は cpu4_weights.md「weightsArray」）。
    //   発火閾値[7]/[9]の動的調整撤去の経緯も cpu4_weights.md を参照。
