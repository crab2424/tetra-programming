...
Object.assign(window.PuyoCPU4.prototype, {
    ...
    //     ・受け量 > 自分の潜在連鎖カバー（直近 bestChain をおじゃま量へ換算）なら fast(カウンター)へ。
    ...
    _updateVersusCounterMode(incomingGross) {
        ...
        if (this._counterActive) {
            ...
            wantCounter = incomingGross > 0 && incomingGross > coverage; // カバーを超えたら発動
        }
        ...
        if (incomingGross > 0 || this._counterActive || modeChanged) {
            ...
            console.log(
                `[cpu4 versus] ${tag}｜受け量=${incomingGross}個 / 自カバー=${coverage}個` +
                (this.cpuMode === 'fast'
