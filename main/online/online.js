 ...
 */
const getHashFromHexString = (hexString) => {
  return hexString.replaceAll("[", "").replaceAll("]", "").replaceAll(" ", "").split(",").map(s => parseInt(s.trim()))
}

const goToOnlineMode = async () => {
  const wtSupported = typeof WebTransport !== "undefined";
  if (!wtSupported) {
    console.error("[ONLINE:Loader]", "WebTransport is not supported in this browser.");
    alert("このブラウザはWebTransportに対応していないため、オンラインモードをプレイできません。\nYou cannot play online mode because your browser does not support WebTransport.");
    return;
  }

  const tetlaboServerUrl = localStorage.getItem("tetlaboServerUrl") || 'https://example.com/online';
  ...
  try {
    connection = new GameConnection(tetlaboServerUrl, tetlaboServerHash ? {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  console.log("[ONLINE:Loader]", "WebTransport server hash:", tetlaboServerHash ? getHashFromHexString(tetlaboServerHash) : null);

  const onlineBtn = /** @type {HTMLButtonElement | null} */ (document.getElementById("online-mode-button"));
  if (onlineBtn) {
